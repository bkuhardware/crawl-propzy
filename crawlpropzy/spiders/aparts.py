...
class ApartsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        request_url = 'https://propzy.vn/mua/ban-can-ho-chung-cu-tphcm'
        request_urls = map(lambda i: request_url + '/p' + str(i), range(1, 65))
        for url in request_urls:
            yield scrapy.Request(url=url, callback=self.parse)
    
    def parse(self, response):
        apart_items = response.xpath('//div[@itemscope]')
        for apart_item in apart_items:
            detail_url = apart_item.xpath('.//a[@itemprop="url"]/@href').extract_first()
            absolute_detail_url = response.urljoin(detail_url)
            yield scrapy.Request(url=absolute_detail_url, callback=self.apart_parse)
    
    def apart_parse(self, response):
        ##title part
